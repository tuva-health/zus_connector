with attending_provider as (
    ...
    inner join {{ ref('stg_encounter_participant_type_coding') }} as eptc
...
, base as (
    ...
    left join {{ ref('stg_zus_encounter_type') }} as et
        on e.encounter_type_id = et.id
    left join {{ ref('stg_zus_encounter_hospitalization_discharge_disposition_coding') }} as ehddc
    ...
    left join attending_provider as ap
        on e.encounter_id = ap.encounter_id
    left join {{ ref('stg_zus_practitioner') }} as zp
        on ap.individual_practitioner_id = zp.practitioner_id
    left join {{ ref('stg_zus_location') }} as zl
